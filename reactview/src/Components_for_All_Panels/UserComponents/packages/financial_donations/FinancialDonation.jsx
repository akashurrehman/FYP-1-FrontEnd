...
import SingleFinancialDonation from "./SingleFinancialDonation";
import packageService from "../../../../Services/Api/User/PackageService";

const FinancialDonation = () => {

    const [financialDonations, setFinancialDonations] = React.useState([]);

    const getData = () => {
        packageService
            .getFinancialDonations()
            .then((data) => {
                setFinancialDonations(data);
            })
            .catch((err) => {
                console.log(err);
        });
    ...
    console.log(financialDonations.results);
    ...
    return ( <div>
        ...
        <div style={{marginTop:'9%',marginBottom:'3%'}}>
            <Container style={{textAlign:'center',width:'50%'}}>
                <Row>
                    <h2 style={{fontWeight:"bold",color:"rgb(160, 15, 15)",fontFamily:"cursive",}}>Find a available Financial Donors</h2>
        ...
        <div style={{width:'99.1%',marginBottom:'13%'}}>
            {financialDonations.length === 0 ? (
                    ...
                    <Row className="d-flex justify-content-center m-5">
                        ...
                        {financialDonations?.results?.bindings?.map((financialDonation, index) => (
                            <Col sm={4} key={index}>
                                <SingleFinancialDonation key={index} financialDonation={financialDonation} />
                            </Col>
                        ))}
