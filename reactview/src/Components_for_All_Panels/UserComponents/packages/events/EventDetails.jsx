...
const EventDetails = () => {
    ...
    const getData = () => {
        packageService
            .getSingleEvent(id)
            .then((data) => {
                setEvent(data?.results?.bindings?.[0]);
            })
            .catch((err) => {
                console.log(err);
        });
        ...
        <div style={{marginTop:'9%',marginBottom:'4%'}}>
            <Container style={{width:'60%'}}>
                <Row style={{textAlign:'center',marginBottom:'5%'}}>
                    <Col sm={12}>
                        <h2 style={{fontWeight:"bold",color:"rgb(160, 15, 15)",fontFamily:"cursive",}}>Event Details</h2>  
            ...
            <Container>
                <Row style={{marginBottom:'10%'}}>
                    <Col sm={6}>
                        <h4 style={{fontSize:'20px'}}>Event Name: <spam style={{fontSize:'16px',fontWeight:'400'}}>{event?.Name?.value}</spam></h4>
                        <h4 style={{fontSize:'20px'}}>Event Location: <spam style={{fontSize:'16px',fontWeight:'400'}}>{event?.Location?.value}</spam></h4>
                        <h4 style={{fontSize:'20px'}}>Date: <spam style={{fontSize:'16px',fontWeight:'400'}}>{event?.Date?.value}</spam></h4>
                        <h4 style={{fontSize:'20px'}}>Event Details: <spam style={{fontSize:'16px',fontWeight:'400'}}>{event?.Message?.value}</spam></h4>
